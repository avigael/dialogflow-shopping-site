...
class App extends React.Component {
  ...
  render() {
    return (
      <>
        <Router>
          <div style={{ height: "100vh" }}>
            <Navbar bg="danger" variant="dark" fixed="top">
              <Link
                to={localStorage.getItem("username") ? "/" + localStorage.getItem("username") : "/"}
              >
                <Navbar.Brand>DialogFlow Shop</Navbar.Brand>
              </Link>
              <Nav className="ml-auto">
                {localStorage.getItem("username") ? (
                  <Link to={"/" + localStorage.getItem("username") + "/cart"}>
                    <Navbar.Text className="mr-sm-2">Cart</Navbar.Text>
                  </Link>
                ) : (
                  <></>
                )}
              ...
              <Route path="/signUp">
                <SignUp
                  signUp={(username, password, firstName, lastName) =>
                    this.signUp(username, password, firstName, lastName)
                  }
                />
              </Route>
              <Route path="/signIn">
        ...
        <div
          style={{ display: "flex", position: "fixed", right: 0, bottom: 0 }}
        >
    ...
    while (this.state.shouldUpdate) {
      try {
        ...
        let response = await fetch(
          window.$ENDPOINT_URL + "/application",
          requestOptions
        );
        if (!response.ok) {
          throw new Error();
  ...
  async fetchCategories() {
    ...
    let response = await fetch(
      window.$ENDPOINT_URL + "/categories/",
      requestOptions
    );
    let result = await response.json();
